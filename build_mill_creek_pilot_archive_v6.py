#!/usr/bin/env python3
"""
build_mill_creek_pilot_archive.py

One-file Mill Creek archive pilot builder.
...
This is designed for a lunch-length timing run, not the final 5,000+ artifact build.
...
def generate_one(assignment: dict[str, Any], args: argparse.Namespace, options: dict[str, Any]) -> tuple[dict[str, Any] | None, dict[str, Any]]:
    ...
    for attempt in range(1, args.max_retries + 2):
        ...
        if not warnings or not args.retry_on_validation:
            return build_record(assignment, generated, metrics, warnings), metrics
    ...
    return build_record(assignment, last_generated, metrics, last_warning_list), metrics
...
def make_medium_manifest(category: str, requested: int, seed: int = 1776) -> list[dict[str, Any]]:
    ...
    global _FORCED_YEAR_QUEUE
    _FORCED_YEAR_QUEUE = even_year_sequence(years_for_medium_category(category), requested)

    rows = [builder(rng, idx) for idx in range(1, requested + 1)]
    _FORCED_YEAR_QUEUE = []
...
def make_all_manifests(categories: list[str], per_category: int, counts_by_category: dict[str, int]) -> dict[str, list[dict[str, Any]]]:
    ...
    for category in categories:
        ...
        if requested <= len(curated_rows):
            manifests[category] = curated_rows[:requested]
        else:
